...
namespace HelloWorld\Scheduler\Helpers;
...
use HelloWorld\Scheduler\Helper;
...
class HlblockHelper extends Helper
{
     ...
     * @return int|void
     */
    public function updateElement($hlblockName, $id, $parameters)
    {
        $dataManager = $this->getDataManager($hlblockName);

        try {
            $result = $dataManager::update($id, $parameters);
    ...
    public function getElements($hlblockName, $params = [])
    {
        $dataManager = $this->getDataManager($hlblockName);
        try {
            return $dataManager::getList($params);
        } catch (Exception $e) {
            $this->throwException(__METHOD__, $e->getMessage());
        }
    }
